...
vec3 CalculateDiffuse(Light light, Material material, vec3 lightDir, vec3 attenuation, float cutOff, float directionalIntensity){
    float diff = max(dot(vNormal, lightDir), 0.0);
    vec3 diffuse = diff * light.Color * diffuseMap * light.Diffuse * Material.Diffuse * attenuation * cutOff * directionalIntensity;
    return diffuse;
}
vec3 CalculateSpecular(Light light, Material material, vec3 lightDir, vec3 attenuation, float cutOff, float directionalIntensity){
    vec3 viewDir = normalize(ViewPosition - vec3(vWorldPosition.xyz));
...
vec3 CalculateLight(Light light) {
    ...
    vec3 ambient = CalculateAmbient(light, Material);
    vec3 diffuse = CalculateDiffuse(light, Material, lightDir, attenuation, cutOff, directionalIntensity);
    vec3 specular = CalculateSpecular(light, Material, lightDir, attenuation, cutOff, directionalIntensity);
